...
export default function RareFindsPage() {
  ...
  useEffect(() => {
    ...
    setLoadError('');
    store.products
      .list({ limit: 100, sort: 'createdAt', order: 'desc' })
          ...
          quickDashProducts
            .filter((product) =>
              product.tags?.some((tag) => {
                ...
                return (
                  normalized === 'rare' || normalized === 'rare-find' || normalized === 'rare-finds'
                );
              })
            )
            .map((product) => ({
                        ...
                        <div className="min-w-[180px] rounded-xl border border-white/10 bg-black/95 p-2 shadow-xl backdrop-blur-xl">
                          {sortOptions.map((option, index) => (
                            <motion.button
                              ...
                              className={`block w-full rounded-lg px-3 py-2 text-left text-sm transition-colors hover:bg-white/10 ${sortBy === option.value ? 'text-white' : 'text-white/70'}`}
                      ...
                      <div className="flex items-center gap-4">
                        <label className="flex cursor-pointer items-center gap-2">
                          <button
                            ...
                            className={`flex h-6 w-11 items-center rounded-full p-0.5 transition-colors ${inStockOnly ? 'bg-white' : 'bg-white/20'}`}
                          >
                            <span
                              className={`h-5 w-5 rounded-full transition-all ${inStockOnly ? 'translate-x-5 bg-black' : 'translate-x-0 bg-white/60'}`}
                            />
                          </button>
                        ...
                        <label className="flex cursor-pointer items-center gap-2">
                          <button
                            ...
                            className={`flex h-6 w-11 items-center rounded-full p-0.5 transition-colors ${onSaleOnly ? 'bg-white' : 'bg-white/20'}`}
                          >
                            <span
                              className={`h-5 w-5 rounded-full transition-all ${onSaleOnly ? 'translate-x-5 bg-black' : 'translate-x-0 bg-white/60'}`}
                            />
                          </button>
                          <span className="font-[family-name:var(--font-inter)] text-sm text-white/70">
                            On Sale
