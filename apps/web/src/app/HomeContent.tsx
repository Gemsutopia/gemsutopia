...
export default function HomeContent({
  ...
  const [testimonials, setTestimonials] =
    useState<{ id: number; name: string; text: string; rating: number }[]>(initialTestimonials);
  ...
  const handleContentUpdate = useCallback(
    (eventData: { section?: string; key?: string; value?: string; refetch?: boolean }) => {
      if (
        eventData?.section &&
        eventData?.key &&
        ...
        setContent((prev) => {
          const idx = prev.findIndex(
            (c) => c.section === eventData.section && c.key === eventData.key
          );
          if (idx >= 0) {
            const updated = [...prev];
  ...
  const heroImages =
    getJsonContent<{ id: number | string; name: string; image: string }[]>('hero', 'images') || [];
    ...
    <div className="flex min-h-screen flex-col overflow-x-hidden bg-black">
      ...
      {marqueeProducts.length > 0 && (
        <div className="relative z-10 min-h-screen overflow-hidden py-20">
          ...
          {featuredTitle && (
            <h2
              className="relative z-10 mb-12 px-4 text-center font-[family-name:var(--font-bacasime)] text-3xl text-white sm:text-4xl md:text-5xl"
          ...
          <div
            className="pointer-events-none absolute inset-0 z-20 opacity-50 sm:opacity-100"
            style={{
              background:
      ...
      {(aboutTitle || aboutText) && (
        <div className="relative z-30 px-2 py-12 sm:px-16 sm:py-16 md:px-32">
          <div
            className="pointer-events-none absolute -top-[150px] -right-[100px] z-50 h-[600px] w-[600px] rotate-[-30deg]"
            style={{
              background:
                'radial-gradient(ellipse 70% 50% at center, rgba(255,255,255,0.1) 0%, transparent 70%)',
            }}
          />
          <div className="flex w-full flex-col-reverse items-center justify-between gap-8 md:flex-row md:gap-20">
            <div className="flex h-auto flex-1 flex-col justify-center px-4 sm:h-[28rem] sm:px-0 md:h-[36rem]">
              {aboutTitle && (
                <h2
                  className="mb-6 font-[family-name:var(--font-bacasime)] text-3xl text-white sm:text-4xl md:text-5xl"
                  style={{ textShadow: '0 0 40px rgba(255,255,255,0.5)' }}
                >
                  {aboutTitle}
                </h2>
              ...
              {aboutText &&
                aboutText.split('\n\n').map((paragraph, i) => (
                  <p
                    key={i}
                    className="mb-4 font-[family-name:var(--font-inter)] text-base leading-relaxed text-white/80 sm:text-lg"
                  >
                    {paragraph}
                  </p>
                ))}
            ...
            {aboutImage && (
              <div className="w-full flex-shrink-0 md:w-auto">
                <img
                  src={aboutImage}
                  alt={aboutTitle || ''}
                  className="aspect-square h-auto w-full rounded-2xl object-cover sm:h-[28rem] sm:w-[28rem] md:h-[36rem] md:w-[36rem]"
                />
              </div>
            )}
          </div>
      ...
      {(earthTitle || earthText) && (
        <div className="relative z-30 px-2 py-20 sm:px-16 sm:py-32 md:px-32">
          <div
            className="pointer-events-none absolute -top-[150px] -left-[100px] z-50 h-[600px] w-[600px] rotate-[30deg]"
            style={{
              background:
                'radial-gradient(ellipse 70% 50% at center, rgba(255,255,255,0.1) 0%, transparent 70%)',
            }}
      ...
      {(faqTitle || faqItems.length > 0) && (
        <div className="relative z-30 px-4 py-12 sm:px-16 sm:py-20 lg:px-24 xl:px-32">
          {faqTitle && (
            <h2
              className="mb-10 text-center font-[family-name:var(--font-bacasime)] text-3xl text-white sm:text-4xl md:text-5xl"
              style={{ textShadow: '0 0 40px rgba(255,255,255,0.5)' }}
            >
              {faqTitle}
            </h2>
            ...
            <div className="mx-auto max-w-3xl lg:max-w-none">
              <Accordion
                ...
                itemClasses={{
                  ...
                  content:
                    'pt-0 pb-4 text-white/70 font-[family-name:var(--font-inter)] text-sm sm:text-base leading-relaxed',
                }}
              >
                {faqItems.map((item) => (
                  <AccordionItem
      ...
      {testimonials.length > 0 && (
        <div className="relative z-10 bg-black py-12 sm:py-20">
          <div
            className="pointer-events-none absolute right-0 bottom-0 left-0 z-10 h-12 sm:h-24"
            style={{ background: 'linear-gradient(to top, rgba(0,0,0,1) 0%, transparent 100%)' }}
          />
          {reviewsTitle && (
            <h2
              className="mb-12 px-8 text-center font-[family-name:var(--font-bacasime)] text-3xl text-white sm:text-4xl md:text-5xl"
              style={{ textShadow: '0 0 40px rgba(255,255,255,0.5)' }}
            >
              {reviewsTitle}
            </h2>
          ...
          <div className="mt-10 text-center">
            <Button
              ...
              className="h-10 rounded-md bg-white/10 px-8 font-[family-name:var(--font-inter)] text-base text-white transition-all duration-200 hover:bg-white/20 sm:h-11 sm:rounded-lg sm:px-10 sm:text-lg"
            >
              Leave a Review
            </Button>
          </div>
      ...
      {(ctaTitle || ctaText) && (
        <div className="relative z-10 overflow-hidden bg-black px-8 py-20 sm:px-16 sm:py-32 md:px-32">
          ...
          <div
            className="pointer-events-none absolute inset-0 z-5 opacity-50 sm:opacity-100"
            style={{
              background:
                'radial-gradient(ellipse at center, transparent 0%, transparent 50%, rgba(0,0,0,0.8) 100%)',
            }}
          ...
          <div className="relative z-10 mx-auto max-w-4xl text-center">
            ...
            {ctaText && (
              <p className="mx-auto mb-10 max-w-2xl font-[family-name:var(--font-inter)] text-lg text-white/70 sm:text-xl">
                {ctaText}
              </p>
            )}
            <div className="flex flex-col justify-center gap-4 sm:flex-row">
              ...
              <Link href="/contact-us" className="w-full sm:w-auto">
                <Button
                  variant="outline"
                  className="h-10 w-full rounded-md border-transparent bg-white/10 px-8 font-[family-name:var(--font-inter)] text-base text-white transition-all duration-200 hover:bg-white/20 sm:h-11 sm:w-auto sm:rounded-lg sm:px-10 sm:text-lg"
                >
                  Contact Us
                </Button>
              </Link>
